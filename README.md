...
## 3. 학습데이터
전체 데이터 12,235건에서 업데이트 날짜별 비율을 반영하여 2,000건을 추출한 뒤, 긍정 데이터를 1로, 부정 데이터를 0으로 라벨링하고 중립 데이터를 제거하여 총 1,862건의 학습 데이터를 생성하였다. 이를 활용해 모델을 학습한 결과, 학습 정확도가 54%, 검증 정확도가 57%로 낮아 모델 성능이 부족하다고 판단하였다. 라벨링 오류 가능성을 고려해 ChatGPT를 활용해 데이터를 재라벨링한 뒤 1,204건을 추출하여 학습을 진행했다. 이 과정에서 ChatGPT가 추출한 1,204건과 직접 라벨링한 1,862건 중 겹치는 1,204건에서 101건의 라벨이 불일치하는 것으로 확인되었다. 그러나 재라벨링 데이터로 학습한 결과, 검증 정확도에는 큰 변화가 없었다. 이에 Electra 논문을 참고해 하이퍼파라메터를 아래 표와 같이 수정한 후 재학습을 수행한 결과, 학습 정확도는 99% 검증 정확도는 87%로 크게 향상되며 유의미한 모델 성능을 확보할 수 있었다.[[Electra 논문 링크](https://arxiv.org/pdf/2003.10555)]
